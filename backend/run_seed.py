"""
Quick seed – avoids Unicode console issues on Windows.
...
async def main():
    async with httpx.AsyncClient(base_url=BASE, timeout=30.0) as client:
        ...
        for v in VOLUNTEERS:
            r = await client.post("/volunteers/", json=v)
            status = "OK" if r.status_code == 201 else f"SKIP({r.status_code})"
            print(f"  {status}  {v['name']}")

        print("\n=== Seeding Reports ===")
        for rep in REPORTS:
            r = await client.post("/reports/", json=rep)
            status = "OK" if r.status_code == 201 else f"ERR({r.status_code})"
            d = r.json() if r.status_code == 201 else {}
            sig_id = str(d.get("signal_id", ""))[:8]
            print(f"  {status}  [{rep['issue_type']:10}]  signal={sig_id}...")

        print("\n=== Resulting Fused Signals ===")
        r = await client.get("/signals/")
        for s in r.json():
            print(f"  [{s['issue_type']:12}] priority={s['priority_score']:.3f}  reports={s['report_count']}  status={s['status']}")
